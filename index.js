...
async function initDBWithRetry(maxRetries = 20, delayMs = 3000) {
  ...
  // Не завершаем процесс, чтобы Railway не уходил в restart-loop и 502
  console.error('DB init failed after retries. Service is up; API will return 503 until DB is back.');
}
...
// Быстрый health без DB-запроса для стабильного healthcheck Railway
app.get('/health', (_, res) => res.status(200).json({ ok: true }));

// Отдельный readiness с DB-проверкой
...
// Если DB пока не готова, API возвращает 503
...
app.post('/api/auth/login', async (req, res) => {
  ...
  try {
    ...
    return res.json({ token, username: user.username, pat, threshold: user.api_limit_threshold });
  } catch (e) {
    console.error(e);
    return res.status(500).json({ error: 'Ошибка сервера' });
  }
...
app.patch('/api/gists/:gistId/name', auth, async (req, res) => {
  ...
  try {
    await pool.query('UPDATE user_gists SET name = $1 WHERE user_id = $2 AND gist_id = $3', [
      name.trim(),
      req.user.id,
      req.params.gistId,
    ]);
